        ...
        children: [
          Expanded(
            child: ListView.builder(
              ...
              itemCount: chatsViewModel.getMessagesForKey(chatKey).length,
              itemBuilder: (BuildContext context, int index) {
                return MessageBubble(
                  message: chatsViewModel.getMessagesForKey(chatKey)[index],
                );
              },
            ),
          ),
                  ...
                  onPressed: () async {
                    ...
                    if (messageText.isNotEmpty) {
                      final response = await _chatService.sendMessage(
                          message: messageText,
                          receiverId: int.parse(chatKey.split('!##!')[0]));
                      if (response != null) {
                        chatsViewModel.addMessageToChat(chatKey, response);
